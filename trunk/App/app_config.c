...
void app_configInit(void)
{
	...
	if (app_configRead(SYSTEM_SETTING_SECTOR) == FALSE
		|| (s_System.Function != SET_FUN_TIME && s_System.Function != SET_FUN_POWER)
		|| (s_System.Refund > 1)
		|| (s_System.Sector > 15 || s_System.Sector < 1)
		|| (s_System.IMaxValue < 1 || s_System.IMaxValue > 9)
		|| (s_System.OverCurrentMultiple <1 || s_System.OverCurrentMultiple >9)
		|| (s_System.RecoveryOldCard > 1)
		|| (s_System.Money < 10 || s_System.Money > 200)
		|| (s_System.Time< 30 || s_System.Time > 600)
		|| (s_System.BaseCurrent < 100 || s_System.BaseCurrent > 900)
		)
	{
		memcpy(&s_System, &s_FactorySystem, sizeof(s_System));
        memset(u8_ledDisBuff, 13, 6);
